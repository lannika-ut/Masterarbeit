...
class parameter:
    ...
    def __init__(self, domain, layer_params=None):
        ...
        if layer_params:
            ...
            self.d_i = fem.Constant(domain, PETSc.ScalarType(4.23e-4)) # m
            self.r_i = fem.Constant(domain, PETSc.ScalarType(0.06*self.d_i.value)) # m
            self.r_w = fem.Constant(domain, PETSc.ScalarType(1.35*self.d_i.value)) # m
            self.rho_s = fem.Constant(domain, PETSc.ScalarType(489)) # kg/m^3
        ...
        self.SSA_0 = fem.Constant(domain, PETSc.ScalarType(708)) # 1/m aber die Zahl hab ich mir erstmal nur so grob hergenommen
    ...
    def assign_material(self, domain, layer_params):
        ...
        Returns:
            dict: dictionnary of the parameter functions of d_i, rho_s, r_i=0.06*d_i, r_w=1.35*d_i and van Genuchten parameters alpha, N using the parametrization from Ymaguchi et al. (2012).
        """
        ...
        for c, x in enumerate(midpoints):
            ...
            for value in layer_params.values():
                if value["locator"](x):
                    ...
                    alpha_vals[c] = (4.4e6)*(value["rho_s"] / value["d_i"])**(-0.98)
                    n_vals[c] = 1 + (2.7e-3)*(value["rho_s"] / value["d_i"])**(0.61)
    ...
    def S_e(self, h_w):
        return ufl.conditional(h_w < 0, (1 + (- self.alpha * h_w)**self.N)**((1 - self.N) / self.N), 1)
    ...
    def K_s(self, phi):
        return 3*(self.d_i / 2)**2 * ufl.exp(-0.013*self.rho_i*(1 - phi))*self.rho_w*self.g / self.mu_w
    
    # interface temperature
    def T_int(self, T_i, T_w, T_intold):
        rho = ufl.conditional(T_w < T_intold, self.rho_w, self.rho_i)
        a1 = self.K_i/self.r_i * T_i + self.K_w/self.r_w * T_w
        a1 += rho * self.L_sol * self.R_m * self.T_melt
        a2 = self.K_i/self.r_i + self.K_w/self.r_w + rho*self.L_sol* self.R_m*self.T_melt
        return a1/a2
