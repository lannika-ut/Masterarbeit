...
class Parameter:
    ...
    def theta(self, Se, phi):
        ...
        t = self.S_l*phi + (0.9*phi-self.S_l*phi)*Se
        return t
    ...
    def W_SSA(self, Se, phi):
        ...
        thetar = self.S_l*phi
        condition_expr = ufl.ge(self.theta(Se, phi), thetar)
        phi0 = 1 - self.rho_s / self.rho_i
        t = self.theta(Se, phi)-thetar
        return ufl.conditional(
            condition_expr,
    ...
    def theta_numerical(self, Se, phi):
        ...
        t_r = self.S_l.value*phi_arr
        t = (t_r + (0.9*phi_arr-t_r)*Se)
        return t
